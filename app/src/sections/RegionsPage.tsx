        ...
        <div className="mt-10 bg-white rounded-xl shadow-sm border p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 items-stretch">
            {[
              ...
              <div key={dim.label} className={`${dim.bg} rounded-lg p-4 border flex flex-col min-h-0`}>
                <dim.icon className={`w-6 h-6 ${dim.color} mb-2 flex-shrink-0`} />
                <p className="font-bold text-[#1C2A33] text-sm leading-tight">{dim.label}</p>
                <p className="text-xs text-[#C9963B] font-semibold mb-1">Weight: {dim.weight}</p>
                <p className="text-xs text-gray-500 line-clamp-2 leading-relaxed">{dim.desc}</p>
              </div>
            ))}
          </div>
